...
function generatePrompt(q: Quote): string {
  const features = q.features?.length
    ? q.features.map((f) => FEATURE_LABELS[f] ?? f).join(", ")
    : "Ninguna adicional";

  const colors = q.brand_colors?.length
    ? q.brand_colors.join(", ")
    ...
    <div className="min-h-screen bg-neutral-50">
      ...
      <header className="bg-white border-b border-neutral-100 px-6 py-4 flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-neutral-900 text-sm">Wo<span className="text-orange-500">xi</span> <span className="font-normal text-neutral-400">/ {quote.business_name || "Cotización"}</span></span>
        </div>
      </header>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-neutral-100 p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {([
                ...
                ["Fecha", new Date(quote.created_at).toLocaleDateString("es-CO", { day: "2-digit", month: "long", year: "numeric" })],
          ...
          <div className="bg-white rounded-2xl border border-neutral-100 p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-3 mb-4">
              {([
                ["Tipo", SITE_LABELS[quote.site_type] ?? quote.site_type],
            ...
            {quote.notes && (
              <div className="mt-4 pt-4 border-t border-neutral-50">
                ...
                <p className="text-sm text-neutral-700 leading-relaxed">{quote.notes}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-neutral-100 p-6">
            ...
            {quote.logo_url && (
              <div className="mb-4">
                ...
                <a href={quote.logo_url ?? "#"} target="_blank" rel="noopener noreferrer"
            ...
            {!quote.logo_url && !quote.brand_colors?.length && (
          ...
          <div className="bg-orange-50 rounded-2xl border border-orange-100 p-6">
            ...
            <p className="text-2xl font-extrabold text-neutral-900">
              {quote.estimate_min
                ? `${formatCOP(quote.estimate_min)} — ${formatCOP(quote.estimate_max)}`
                : "No calculado"}
            </p>
            {quote.currency && <p className="text-xs text-neutral-400 mt-1">Moneda cotizada: {quote.currency}</p>}
          </div>
        </div>
